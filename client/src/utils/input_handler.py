...
from .audio_file import send_audio
from ..mtypes import Status, Cosmic
from ..config import ClientConfig as Config

__all__ = ["bond_shortcut"]
...
def shortcut_correct(e: keyboard.KeyboardEvent):
    ...
    if e.name is None:
        return False
    key_expect = normalize_name(Config.shortcut).replace("left ", "")
...
def launch_task():
    ...
    asyncio.run_coroutine_threadsafe(
        Cosmic.queue_in.put({"type": "begin", "time": t1, "data": None}), loop
    )
